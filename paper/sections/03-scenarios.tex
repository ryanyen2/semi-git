...
All five scenarios below fail for the same reason: the developer names their
work by the request that produced it (``the caching,'' ``the rate limiter''),
because that is the level their understanding of the code is pitched
at~\cite{naur1985}, while the record they have to operate through holds lines
inside commits (Figure~\ref{fig:granularity}). Every scenario has a workaround
and all the workarounds work. The argument is about what the workarounds cost,
and what they cost is recovering from the code which function served which
request---a decomposition the developer stated in their own words before any code
existed, and that nothing wrote down.

The five scenarios follow one developer through a single afternoon and the
week after it. The project is a Python service that answers price quotes for a
trading dashboard; its request handling lives in \sym{api.py::handle}, its database
access in \sym{db.py::query}, and the developer asked a coding agent for three
things in one sitting: a rate limiter, a price cache, and a retry policy.
...
boundary does exist at that moment. The developer stated the boundary themselves
before any of the code existed. What they lack is the list of functions on either
side of it, which only the agent ever had.
...
hunks belonging to three other purposes. One cache edit adds a parameter to
\sym{db.py::query}; another passes that parameter from \sym{api.py::handle}. Three
...
decomposition that nobody recorded, and that recovery was affordable when the
person paying it was the person who had written the code---because they held the
answer already, having typed the lines minutes earlier. A developer who typed
``add a retry policy'' instead does not hold the answer, and the longer the session
