...
came out of \sgt{} as it is today. Output lines too wide for this column are wrapped onto a second
line and are otherwise unchanged. Read the sequence for which decisions the
...
already written. The developer read it, agreed the sitting had been mixed, and carried on,
which is the case Section~\ref{sec:scenarios} opened with.
...
said anything about that part of the work, which is the failure
...
~~reverting this removes 11 edits \ttdash{} 7 of them work built on top
\end{out}
...
developer to guess that four was the right number of places to look. The developer
did not ask for the last line of that output and read it anyway, and it says
that seven later edits are recorded as built on this rate limiting, so a sliding
window is a change underneath work that already depends on it, and the developer
knows that before opening the file.
...
~operate:~~sgt revert 4a1c9e02:sliding-window\\
~~~~~~~~~~(one checkpoint, by name)~~\ttdot{}\\
...
leaves Monday's four edits in place. The last two lines exist because we watched
...
that function keeps the retry work and loses the cache's three lines, which is
how removing a week old request leaves a change made yesterday in place. Two
functions existed only for the cache
and go away with it, which empties \sym{cache.py} and removes the file, and
...
lookup from each of them. A revert can be taken back exactly, and that property is
why we build every one of these operations as set arithmetic, so the developer
tested it.
...
\noindent
Putting the cache back produced the same bytes that were in the files before the
revert, and it kept the three call site repairs the developer had made in between, because
both directions are set arithmetic over the same records and neither one reapplies
...
with it, which is the fifth operation from
Section~\ref{sec:scenarios}. The record of the call to \sym{api.py::\_bucket} was
...
~~~~~~api.py::handle: sgt merge-op 3c91ab02 8ee14d77
\end{out}

\noindent
That last line names \cmd{sgt merge-op}, which takes two edit identifiers and is
not the command a person should be reaching for. \cmd{sgt log} in the same state
prints \cmd{api.py::handle \ttarrow{} sgt resolve api.py::handle}, which is the
command to use. Two
commands telling the developer different things to type is an inconsistency we
have not cleaned up. The two
...
so git merges them without comment, which is the case
...
that. The case we designed for is the one where the merge is wrong, where one edit
adds an early return and the other edit's new code sits below it, and the test run
that would have caught it happens on a branch nobody has pushed yet. A developer
...
code, which is where we wanted them.
...
had read, so their test suite is what decided the revert had worked. The fork
