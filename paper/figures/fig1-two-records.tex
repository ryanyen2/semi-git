...
developer stated three requests and the agent made a fourth change nobody asked
for. Git stored two commits whose fifteen hunks interleave all four, and the
colours in that panel are ours, since git holds no record of which request
produced which hunk. \sgt{} stored one edit per changed function under the request
it came from, so the caching is six edits with a name.}
...
\newcommand{\shared}[2]{\draw[color=sgtInk!45, line width=0.3pt, densely dotted]
  (#1,#2) circle (2.0);}
...
% what the rings mean
\annot{142.8}{45.4}{140.0}{43.0}{20}{sgtInk}
