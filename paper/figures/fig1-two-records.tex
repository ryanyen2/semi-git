...
under each, and each commit holds hunks of three different colours, so two
requests and the unrequested change are mixed together inside each of them. The
...
under git, and one named command under sgt. The request's name is set in its own
colour both where the task is stated and inside the sgt command, because they are the
same words.}
...
% The request is named in its own colour here and again in the command on the right,
% because those are the same words and the reader typing them is the whole argument.
% The middle column has no name to colour, which is the thing git does not store.
\node[anchor=north west, text width=32mm, align=left, inner sep=0, text=sgtInk,
  ...
  {A week later, the \textcolor{fCache}{price cache} has to come back out.};
