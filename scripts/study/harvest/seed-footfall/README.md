...
A small dashboard over the Spencer Street pedestrian counter in Melbourne.

The city has sensors on both sides of the crossing between Southern Cross station
and the Collins Street offices, counting people every hour since 2013. The council
publishes the file and we use it for the transport committee's quarterly paper.
...
Put a module in `footfall/pages/`. Give it `PATH`, `TITLE`, `ORDER` and a
