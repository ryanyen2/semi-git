...
export type RequestId =
  ...
  | 's1' | 's2' | 's3' | 's4'
  /**
   * Retired. `d1`-`d4` were the locate-and-reverse cards of protocol v1,
   * `w1`-`w3` the feature-removal block, `r1`-`r3` the three-request design,
   * `r4`-`r6` the six-request one before it, and `f1`/`f2` the two standalone
   * reach trials. Kept so pilot documents still read: narrowing this to the
   * live set would make every read of a pilot document a lie the compiler
   * endorses, and would make `requestById`'s undefined branch -- the one that
   * stopped the dashboard throwing mid-render -- look unreachable to whoever
   * reads it next.
   */
  | 'd1' | 'd2' | 'd3' | 'd4'
  | 'w1' | 'w2' | 'w3'
...
export interface RequestDoc {
  ...
  /**
   * When the working phase of a stage ended and the untimed answer phase
   * began. Protocol v2 stages run in two phases: the capped work, then the
   * quiz and ratings with no clock. `elapsedMs`/`activeMs` cover the work
   * phase only, and are written when this is. Absent on v1 documents.
   */
  workEndedAt?: number | null
  /**
   * The stage quiz, answered after the work with no clock running.
   * Keyed by quiz item id. Behaviour checklists store arrays of behaviour
   * ids; choices store the option value; free text stores the string.
   * Scored against the answer key after the session, never in the browser.
   */
  quiz?: Record<string, string | string[] | null>
  /**
   * The three per-stage rating statements, 1-7, keyed by rating id.
   * These are protocol v2's replacement for the HLAC battery: the same kind
   * of item, asked in the minute after the experience it asks about.
   */
...
export interface GroundTruth {
  ...
  requestKeys: Record<
    string,
    {
      ...
      /**
       * Behaviour ids the named work reaches, or -- for a stage quiz's
       * checklist -- the behaviours the stage's change actually moves. Present
       * on the steps that carry a checklist, and generated by the measuring
       * scripts in scripts/study/ rather than written by hand: a designer
       * choosing this set would be choosing the result. Either one shared
       * list or one per project.
       */
      reach?: string[] | Partial<Record<string, string[]>>
      /**
       * Correct option values for scored multiple-choice quiz items,
       * item id -> option value. Same in both projects by construction, and
       * the build gate verifies it (protocol v2 section 9).
       */
