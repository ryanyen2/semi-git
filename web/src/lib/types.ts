...
export type RequestId =
  | 'r1' | 'r2' | 'r3'
  /** Retired with the six-request design. Kept so pilot documents still read. */
  | 'r4' | 'r5' | 'r6'
  /**
   * The two reach trials. New ids rather than reusing `r4`/`r5`, which would
   * make one id mean two different things across the collection and give the
   * dashboard no way to tell a pilot's fourth request from a reach trial.
   */
  | 'f1' | 'f2'
...
export interface RequestDoc {
  ...
  /**
   * Reach trials only. The same question answered twice: once from the
   * representation alone, then again after using the setup. `gain` is the
   * difference, and it is the whole point of the trial, so both are stored
   * rather than only the final answer.
   *
   * Picks are behaviour ids, not indices: the list is one shared set of twelve
   * across both trials, so an id survives a re-ordering that an index would not.
   */
  stages?: {
    blind?: ReachStage
    checked?: ReachStage
...
export interface GroundTruth {
  ...
  requestKeys: Record<
    string,
    {
      ...
      /**
       * Behaviour ids the named work reaches. Present on the reach trials only,
       * and generated by scripts/study/measure_reach_key.py rather than written
       * by hand -- a designer choosing this set would be choosing the result.
       */
      reach?: string[]
    }
  >
