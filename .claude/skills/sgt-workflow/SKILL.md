...
(cuts one feature into two, previewed then confirmed — `sgt split <feature> [--apply]`, no
...
| Drop an op but keep dependents alive | `sgt revert <ref> --keep-dependents [--repair]` | drafts a continuation hollow per *direct* reference-dependent so its symbol stays live, instead of cascading the delete through everything downstream. Bare form prints the draft — you (or an agent) must `sgt fulfill <draft-id> --from-tree` then `sgt commit`. `--repair` skips the manual step: hands the draft straight to the LLM repair loop, which fulfills + runs it through the same oracle gate automatically. |
...
- **You want durable per-agent attribution.** `sgt session land <name>` stamps `session=<name>`
  ...
  `sgt review-queue` provenance filtering work later — a scratch worktree that never ran through
...
- **`sgt commit [--message ...]`** — commits a *staged rewrite candidate* (the output of
...
3. **`sgt checkpoint`** previews footprint-overlap matches between the plan's pending steps and
   ops actually mined since intake. Confirm a specific group with `--confirm-hollow <id>...
   --confirm-op <id>...` once you're satisfied the match is right — checkpoint never
   auto-applies.
4. **`sgt drift`** lists ops that were mined but that *no* active plan predicted — the things that
   happened outside any stated plan. Check this before landing/proposing if you want to know
   whether unplanned changes crept in.
5. Once the work is checkpointed (or you're intentionally landing drift too), move to landing:
   `sgt commit` (staged rewrite), `sgt land <branch>` (direct shared advance), or
...
- `sgt preview <verb> <args>` previews merge/rename/move/revert side-effect-free. For split,
  there's no separate preview verb — bare `sgt split <feature>` (no `--apply`) already previews;
  `--apply` confirms it.
- `sgt select <feature>...` / `sgt why <op>` are *explanation* verbs (closure/attribution), not
  a way to switch what's materialized in the working tree — they don't change state.
- `sgt migrate [feature-ids|ops-v3] [--apply]` is a one-time op-store schema migration, not a
  daily-loop verb — dry-run by default, run it only when a plan doc or error message tells you to.
- `sgt fulfill <draft-id> --from-tree` stages a drafted hollow's image; it does not commit.
  `sgt unstage` abandons a staged candidate without committing it. `sgt commit` is the only verb
