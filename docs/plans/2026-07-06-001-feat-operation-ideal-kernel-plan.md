...
- **Status (2026-07-07): DONE.** Both gaps closed and the folded-in `order.py` value-collision bug fixed; full suite green (488 passed, 1 skipped). On-disk form is the **full explicit `op_ids` set** per ref (`.sgt/local/ideal.json`), not the frontier-vector KTD form the Approach above sketched — chosen so Gap 1 never depends on `order.downset`'s still-unhardened value-collision closure (see the FINDINGS.md "U7.5" entry for the rationale). Two consequences for U8 are recorded as **Implementation notes** under U8 below: the `upset`/`downset` hardening prerequisite, and the fact that `put()` does not yet persist an explicitly-edited ideal or advance the witness.
...
- **Implementation notes (added post-U7.5, 2026-07-07 — from the shipped lens/order code, not re-derivable from the original design):**
  - **U8 is the first real caller of `order.upset`/`downset`, and must close their deferred value-collision caveat.** U7.5's Fix 1 hardened `is_valid_ideal`/`frontier` to reason about producers *existentially* (some in-ideal op produces a given `(symbol, version)` pair), but deliberately left `chain_edges`/`reference_edges`/`upset`/`downset` on the old `(symbol, after_version)`-keyed adjacency, which picks *one* producer on a collision — see the in-code `DEFERRED CAVEAT (U7.5)` comments on `upset`/`downset`. `revert X = I \ ↑X`, and `pin`/`restore`/`cherry-pick` compute a `↓X`, so U8 must either **(a)** harden `upset`/`downset` to resolve chain adjacency the fork-free way `frontier` now does — *within the ideal*, `before_version` is a unique key, so an op's successor/predecessor along a symbol's chain is unambiguous — instead of over the whole op universe keyed on `after_version`; or **(b)** prove the collision (two in-ideal ops sharing an `after_version`, e.g. an add→modify→revert) cannot arise for that verb's inputs. **(a)** is the safe default; `tests/laws/corpus.py::revert_to_original` is a ready regression fixture.
  - **A reducing edit (revert/pin) must persist the edited set AND advance the witness — `put()` alone does neither.** `put(repo, edited_ideal)` materializes + commits a witness W, but (i) does not write `edited_ideal` to `.sgt/local/ideal.json`, and (ii) leaves the witness at the pre-`put()` HEAD (its internal `get()` records the pre-edit head). So a naïve `verb → put()` is *unsafe* for a reducing edit: the next `get()` mines W's codebase diff (the reverted change) as a fresh inverse op and unions it onto the still-full persisted base — resurrecting/inverting the edit rather than dropping `↑X`. (The `put∘get` round-trip law is unaffected only because there W's codebase diff is empty — W touches only `.sgt/` — so nothing is re-mined.) U8's apply path must, *after* `put()` commits W: persist `edited_ideal`'s committed op-ids to `.sgt/local/ideal.json[ref]` and set the witness to W, so the next `get()` has `since=W` (mines nothing new) and trusts the edited base. Recommend a small public `lens` helper (e.g. `record_ideal(repo, ideal, witness_sha)`) rather than each verb reaching into the private `_save_ideal_table`/`_save_witnesses`; U7.5 kept those private because no U7.5 test exercised the explicit-edit write path.
  - **Pattern reference is not a dependency.** U8's `plan_*/apply` shape references `sgt/lifecycle/algebra.py`, which U10 deletes — that's a *style* reference only; the verbs live in the new `sgt/core/verbs.py`, so U8 does not resurrect a dependency on a to-be-deleted module.
- **Status (2026-07-07): DONE.** `sgt/core/verbs.py` (revert/pin/restore/cherry-pick/after as pure `plan_*` + gated `apply`), collision-safe `order.upset_in`/`downset_in`, `lens.record_ideal` + declared-edge store, and `api.verb_preview_view`; full suite 497 passed, 1 skipped. Note (a) above was taken, with a **correction**: hardening `upset`/`downset` "the fork-free way `frontier` does" turned out to be insufficient — the existential downward-closure the U7.5 note assumed for `is_valid_ideal` *admits an originless `{modify, revert}` cycle* when a chain head is reverted, which has no frontier head. `is_valid_ideal` was tightened to **grounding** (`order._grounded`, least fixpoint from `before=None` heads); `upset_in = ideal \ _grounded(ideal - {X})`. See the FINDINGS.md "U8" entry. Per the Scope decision, the mutating CLI/MCP flip onto these verbs stays **U10** (only `sgt/core/verbs.py` + `api.verb_preview_view` shipped, not `cli.py` re-pointing).
...
- **Status (2026-07-09): DONE.** Shipped as specified in commit `f000a7b`: `sgt/lens/verbs.py` (merge/split/rename/move + `plan_revert_feature`), `map_view`/`blame_view`/`status_view` in `sgt/api.py`, `sgt map`/`sgt feature *`/`sgt blame`/`sgt status` in the CLI, and a from-scratch TUI (`sgt/tui/app.py`) and VS Code rewrite over the new projection -- the old decision-DAG webview and its activity/codelens/hover machinery were deleted rather than ported, since there is no kernel equivalent of a "decision" node. Byte-neutrality of the metadata verbs holds by construction (`code(I)` never reads tree/pins). Full suite green. This status line and the FINDINGS.md "U13" entry were added retroactively during a later review — the plan doc and FINDINGS.md were not updated when the code shipped.
...
- **Status (2026-07-09): DONE, with two notes.** Shipped as specified: `sgt/loop/{plan,match}.py`, `sgt_plan_intake`/`sgt_checkpoint`/`sgt_drift` MCP tools, `sgt plan`/`sgt checkpoint`/`sgt drift` CLI verbs, `plan_view`/`drift_view` in `sgt/api.py`, and VS Code `src/plan.ts`. Rationale transfer is a side-table join (`plan_matches.json`: op id -> session/hollow/intent) rather than a mutation on the fulfilled `Op`, since ops are immutable and content-addressed by construction — reading intent means joining against this table, not reading the `Op` itself. (1) **Provenance note:** this unit's code, tests, and surfaces landed bundled inside commit `6a1557b`, whose message names only an unrelated `mine.py`/`cli.py` fix; discovered during a later review, not flagged at commit time. (2) **Deferred to U15:** declared order edges (`sgt after`) live in the gitignored `.sgt/local/declared.json`, so they don't currently travel between clones at all — R19's "declared-edge cycles introduced by union are detected and surfaced" presumes they do. U15 must resolve this (promote to a committed path, or an explicit exchange step) or the requirement doesn't apply. See FINDINGS.md's "U14" entry.
...
- **Status (2026-07-09): DONE.** Shipped as specified: `sgt/core/sync.py` (`sync(repo, remote, branch) -> SyncReport`), `sgt/lens/reconcile.py` (`union_pins`, `reconcile_tree`), `order.forks`/`order.find_declared_cycles` (new in `sgt/core/order.py`), transport helpers on `GitBinding` (`fetch`, `merge_ours_no_commit`, `merge_abort`, `complete_merge`, `default_remote`), `sgt sync` (CLI) and `sync_view` (`sgt/api.py`). Closes the U14 FINDINGS gap: declared edges are now committed at `.sgt/declared.json` (was gitignored `.sgt/local/declared.json`), so they travel between clones through git same as ops. One real bug found and fixed during test-writing, not anticipated by the plan: `sync` built the merged `Ideal` from the full unioned declared-edge set *before* checking for cycles, so a genuinely cyclic union (two clones each declaring the opposite order on the same pair) raised instead of being reported — fixed by computing `find_declared_cycles` first and excluding cyclic edges from the ideal fold while still persisting and reporting the full union for `after` retraction. Regression coverage: `tests/core/test_sync.py` (6 tests — AE4 disjoint-edit merge, idempotence/double-mine determinism, same-symbol fork surfaced without committing, cross-clone op-id dedup with unioned provenance, pin contradiction reported while still merging, declared-edge cycle reported with edges traveling). Full suite green.
