...
|---|---|---|
| the first version of the dashboard | `data.py` reads the counter's file, `metrics.py` adds it up, `charts.py` draws, `pages/overview.py` is the front page | **The front page: the busiest day, and the last fortnight chart** ![](/stages/bikecount-overview.png) |
| the hour-of-day page | `pages/hourly.py` draws it, `metrics.py` works out the averages | **The busiest hour, and the hour-of-day chart under it** ![](/stages/bikecount-hourly.png) |
| splitting that page into weekdays and weekends | `pages/hourly.py`, `metrics.py` | **The weekday chart and the weekend chart, side by side** ![](/stages/bikecount-hourly-split.png) |
| the list of unusual days the project keeps | `events.py` | Nothing on its own. It is the list the next two rows read. |
| the month-by-month page | `pages/monthly.py`, `metrics.py` | **The month-by-month chart** ![](/stages/bikecount-monthly.png) |
...
| the east vs west page | `pages/sides.py`, `metrics.py` | **The east against west comparison** ![](/stages/bikecount-sides.png) |
| the by-year table | `pages/yearly.py`, `metrics.py` | **The one-row-per-year table** ![](/stages/bikecount-yearly-plain.png) |
...
| the finding that the quieter sensor is real, not a fault | `metrics.py`, `pages/sides.py`, `pages/yearly.py` | Two more charts on the east vs west page, and the caveat under the by-year table. |
| the date window | `window.py`, `metrics.py`, `server.py`, `pages/__init__.py`, and every page under `pages/` | **The date window at the top of every page** ![](/stages/bikecount-window.png) |
| rounding the front page numbers — the newest work in the project | `pages/overview.py` | The busiest-day figure and the last-fortnight table, to the nearest 10. |
