...
You are the new maintainer of a program called confplan. It is a small
command-line tool that a conference committee uses to plan a two-day program.
Sam Park built it over the last six weeks, partly by working with an AI
assistant, and has now left the team.

You have the code, its full history, and your assistant. Four requests have
come in. Work through them in order. Two more requests are at the end if you
have time.
...
longer join a waitlist, nobody is promoted off it when a seat frees up, and
the seat notices stop.
...
The test suite is your safety net. When you think you are done, run `pytest -q`.
It should pass, except for the waitlist's own tests, which may be gone.

## Request 3: unregistering still needs to work

Do this inside the 15 minutes above.

One correction to the last request: attendees must still be able to
unregister from a session themselves. Bring the unregister command back,
without any waitlist promotion happening when a seat frees up.
...
back-to-back sessions. If an attendee is in a session that runs 09:00 to
10:30, the app now refuses to register them for one that runs 10:30 to 12:00,
calling it a clash. It used to work, and there was even a fix that
specifically made adjacent sessions legal.
...
Attendees want to swap between two sessions of the same talk in one step.
There are two reasonable ways to build it:

- Do the whole swap at once, and if the new session refuses the attendee, put
  them back in the old one.
- Unregister the attendee first and hold their seat for a moment, then
  register them in the new session.
...
The change you looked at in request 1 bothers you. Two unrelated pieces of
work landed as one unit. Separate them in the history so each one has a clear
name. Do not change any of the current code.
