...
The visual surface is the VS Code extension (`editor/vscode`), version 0.6.0. It is **not part of
the sgt CLI** and needs a one-time install before a session.

**One-time setup** (done on this machine; `code --list-extensions` shows `semi-git.semi-git@0.6.0`):

```bash
cd /Users/r4yen/repos/semi-git/editor/vscode
npx --yes @vscode/vsce package --no-dependencies --out /tmp/semi-git-0.6.0.vsix
code --install-extension /tmp/semi-git-0.6.0.vsix
```
