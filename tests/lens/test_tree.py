...
def test_residue_members_rehome_to_their_anchor_entitys_leaf():
    ...
    # holding no real symbol at all. In the study fixtures 4 of 21 features were this shape, named
    # "add course search" / "waitlist for full sections", i.e. the task keywords, showing
    # "0 symbols in 0 files". Residue follows its anchor ENTITY's lane for op assignment already
    # (`_member_leaf_for`); membership must follow the same rule, so the residue-only leaf empties
    # out and `_prune_empty_leaves` drops it.
    nodes = {
        "N0": {"id": "N0", "parent": None, "depth": 0, "members": ["a.py::foo", "a.py::bar"],
               "size": 2, "dir": "pkg", "children": ["N1", "N2"], "split_reason": None},
        "N1": _leaf("N1", "N0", ["a.py::foo", "a.py::bar"], "pkg"),
        # residue-only leaf: every member anchors to an entity that lives in N1
        "N2": _leaf("N2", "N0", ["a.py::__residue__::foo", "a.py::__residue__::bar"], "pkg"),
    }
    roots = ["N0"]
    ...
    assert sorted(nodes["N1"]["members"]) == [
        "a.py::__residue__::bar", "a.py::__residue__::foo", "a.py::bar", "a.py::foo",
    ]


def test_rehome_head_residue_follows_its_files_owning_leaf():
    # A file-head residue names no anchor entity, so it follows the leaf owning the plurality of its
    # file's real symbols. Leaving it in place was the first version of the rule, and it kept all
    # four phantom leaves in the study fixtures alive with exactly one member each.
    sentinel = "a.py::__residue__::\x00HEAD\x00"
    nodes = {
        "N0": {"id": "N0", "parent": None, "depth": 0, "members": [], "size": 0, "dir": "",
               "children": ["N1", "N2"], "split_reason": None},
        "N1": _leaf("N1", "N0", ["a.py::foo", "a.py::bar"], "pkg"),
        "N2": _leaf("N2", "N0", [sentinel], "pkg"),
    }

    tree._rehome_pseudo_members(nodes)

    assert nodes["N2"]["members"] == []
    assert sentinel in nodes["N1"]["members"]


def test_rehome_keeps_a_pseudo_member_whose_file_has_no_real_symbol():
    # No anchor entity and no real symbol of that file anywhere: there is genuinely no lane to
    # follow, and the member is live in the ideal, so it must stay rather than be dropped.
    sentinel = "a.py::__residue__::\x00HEAD\x00"
    nodes = {
        "N0": {"id": "N0", "parent": None, "depth": 0, "members": [], "size": 0, "dir": "",
               "children": ["N1", "N2"], "split_reason": None},
        "N1": _leaf("N1", "N0", ["b.py::foo"], "pkg"),
        "N2": _leaf("N2", "N0", [sentinel], "pkg"),
    }

    tree._rehome_pseudo_members(nodes)

    assert nodes["N2"]["members"] == [sentinel]
...
def test_assign_pin_does_not_alias_a_leaf_that_already_wears_its_id(tmp_path):
    # Regression: the pin's winning leaf and the leaf *currently carrying* the pinned id can be two
    # different leaves -- a previous rebuild renamed the leaf that then held the plurality, and this
    # clustering moved the plurality elsewhere. `_apply_assign_pins` only dropped self-renames, so it
    # renamed the new winner onto an id another live leaf still wore: `_apply_id_map` kept one of the
    # two node dicts and dropped the other leaf's members outright, and the parent listed the id
    # twice, which `_dedup` then resolved by deleting the survivor (KeyError). Found on the confplan
    # study fixture, where `sgt log --refresh` crashed on every run.
    from sgt.lens.pins import Pins

    result = {
        "roots": ["N0"],
        "op_leaf": {"opA": "af-x", "opB": "N2"},
        "nodes": {
            "N0": {"id": "N0", "parent": None, "depth": 0, "members": ["c1", "c2", "c3"],
                   "size": 3, "dir": "pkg", "children": ["af-x", "N2"], "split_reason": None},
            "af-x": _leaf("af-x", "N0", ["c1"], "pkg"),        # the stale holder: 1 pinned member
            "N2": _leaf("N2", "N0", ["c2", "c3"], "pkg"),      # the new plurality: 2 pinned members
        },
    }
    pins = Pins(assign={"c1": "af-x", "c2": "af-x", "c3": "af-x"})
    ...
    assert len(children) == len(set(children))                  # no aliased/duplicate child id
    assert set(nodes) - {"N0"} == set(children)                 # nodes and children stay consistent
    # Nothing is lost: both leaves survive and every member is still held by exactly one of them.
    leaves = [nd for nd in nodes.values() if not nd["children"]]
    assert len(leaves) == 2
    assert sorted(m for nd in leaves for m in nd["members"]) == ["c1", "c2", "c3"]
    # The pin lands on the plurality leaf; the stale holder is renamed off the id, not merged into it.
    assert nodes["af-x"]["members"] == ["c2", "c3"]
